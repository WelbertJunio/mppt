...
void Charging_Algorithm(){
  if(ERR>0||chargingPause==1){buck_Disable();}                                       //ERROR PRESENT  - Turn off MPPT buck when error is present or when chargingPause is used for a pause override
    ...
    if(REC==1){                                                                      //IUV RECOVERY - (Only active for charging mode)
      ...
      lcd.setCursor(0,0);lcd.print("SOLAR POWER     ");                              //Display LCD message
      ...
      lcd.clear();
       ...
       /////////////////////// Float charge ALGORITHM //////////////////////////////
    if(Float_charge == 1 && voltageOutput >= (voltageBatteryFloat - 2.4  )){         // Use 2.4 para 24v ou 1.2 para 12v    
        if(currentOutput>5)                       {PWM--;}                           //Current max = 5A  
        else if(voltageOutput>voltageBatteryFloat){PWM--;}                           //Voltage Is Above → Decrease Duty Cycle   
        else if(voltageOutput<voltageBatteryFloat){PWM++;}                           //Increase duty cycle 
      ...
      if(MPPT_Mode==0){                                                              //CC-CV PSU MODE
        ...
        PWM_Modulation();                                                              //Set PWM signal to Buck PWM GPIO             
        if(voltageOutput >= voltageBatteryMax){Float_charge = 1;}
        else {Float_charge = 0;}
      }
    }
    }
