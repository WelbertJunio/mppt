...
void loadSettings(){ 
  ...
  voltageBatteryFloat  = EEPROM.read(14)+(EEPROM.read(15)*.01);  // Load saved maximum battery voltage setting
  voltageBatteryON   = EEPROM.read(16)+(EEPROM.read(17)*.01);  // Load saved maximum battery voltage setting
}
...
//EEPROM.write(11,flashMemLoad);       //STORE: Enable autoload (must be excluded from bulk save, uncomment under discretion)
  ...
  conv1 = voltageBatteryFloat*100;       //STORE: Maximum Battery Voltage (gets whole number)
  conv2 = conv1%100;                   //STORE: Maximum Battery Voltage (gets decimal number and converts to a whole number)
  EEPROM.write(14,voltageBatteryFloat);
  EEPROM.write(15,conv2);
  conv1 = voltageBatteryFloat*100;       //STORE: Maximum Battery Voltage (gets whole number)
  conv2 = conv1%100;                   //STORE: Maximum Battery Voltage (gets decimal number and converts to a whole number)
  EEPROM.write(16,voltageBatteryON);
  EEPROM.write(17,conv2);
