...
void displayConfig2(){
  ...
  lcd.setCursor(11,0);lcd.print(currentInput,2);lcd.print("A");padding10(currentInput);    
  lcd.setCursor(0,1); lcd.print(batteryPercent);lcd.print("%");padding100(batteryPercent); 
  if(BNC==0){lcd.setCursor(5,1); lcd.print(voltageOutput,1);lcd.print("V");padding10(voltageOutput);}
  else{lcd.setCursor(5,1);lcd.print("NOBAT");}
  lcd.setCursor(11,1);lcd.print(currentOutput,2);lcd.print("A");padding10(currentOutput);    
...
void LCD_Menu(){
  ...
  if(settingMode==1){
    ...
    ///// SETTINGS MENU ITEM: COOLING FAN /////5
    ...
    ///// SETTINGS MENU ITEM: FAN TRIG TEMP /////6
    ...
    ///// SETTINGS MENU ITEM: FIRMWARE VERSION /////12
   ...
   ///// SETTINGS MENU ITEM: voltageBatteryFloat V /////
      ...
      if(setMenuPage==0){floatTemp = voltageBatteryFloat;}
        ...
        if(digitalRead(buttonRight)==1){                                                    //Right button press (increments setting values)
          while(digitalRead(buttonRight)==1){
            if(millis()-currentMenuSetMillis>longPressTime){                                //Long Press     
              voltageBatteryFloat += 1.00;                                                    //Increment by 1
              voltageBatteryFloat = constrain(voltageBatteryFloat,vOutSystemMin,vOutSystemMax); //Limit settings values to a range
              lcd.setCursor(2,1);lcd.print(voltageBatteryFloat,2);delay(longPressInterval);   //Display settings data                               
            }
            else{                                                                           //Short Press  
              voltageBatteryFloat += 0.01;                                                    //Increment by 0.01
              voltageBatteryFloat = constrain(voltageBatteryFloat,vOutSystemMin,vOutSystemMax); //Limit settings values to a range
              lcd.setCursor(2,1);lcd.print(voltageBatteryFloat,2);delay(shortPressInterval);  //Display settings data                            
            }  
            lcd.print("V   ");                                                              //Display unit
          } 
        }
 ...
 ///// SETTINGS MENU ITEM: voltageBattery ON /////
    else if(subMenuPage==6){
      lcd.setCursor(0,0);lcd.print("BATTERY ON      ");
      if(setMenuPage==1){lcd.setCursor(0,1);lcd.print(" >");}
      else{lcd.setCursor(0,1);lcd.print("= ");}
      lcd.setCursor(2,1);lcd.print(voltageBatteryON,2);lcd.print("V");  
      lcd.print("                ");  

      //SET MENU - FLOATTYPE
      if(setMenuPage==0){floatTemp = voltageBatteryON;}
      else{
        if(digitalRead(buttonBack)==1){while(digitalRead(buttonBack)==1){}voltageBatteryON = floatTemp;cancelledMessageLCD();setMenuPage=0;} 
        if(digitalRead(buttonSelect)==1){while(digitalRead(buttonSelect)==1){}saveSettings();setMenuPage=0;savedMessageLCD();}     
        currentMenuSetMillis = millis();
        if(digitalRead(buttonRight)==1){                                                    //Right button press (increments setting values)
          while(digitalRead(buttonRight)==1){
            if(millis()-currentMenuSetMillis>longPressTime){                                //Long Press     
              voltageBatteryON += 1.00;                                                    //Increment by 1
              voltageBatteryON = constrain(voltageBatteryON,vOutSystemMin,vOutSystemMax); //Limit settings values to a range
              lcd.setCursor(2,1);lcd.print(voltageBatteryON,2);delay(longPressInterval);   //Display settings data                               
            }
            else{                                                                           //Short Press  
              voltageBatteryON += 0.01;                                                    //Increment by 0.01
              voltageBatteryON = constrain(voltageBatteryON,vOutSystemMin,vOutSystemMax); //Limit settings values to a range
              lcd.setCursor(2,1);lcd.print(voltageBatteryON,2);delay(shortPressInterval);  //Display settings data                            
            }  
            lcd.print("V   ");                                                              //Display unit
          } 
        }
